...
def se_orbit(semimajoraxis:float, eccentricity:float=0, inclination:float=0, angle:float=0, retrograde:bool=False) -> [str]:
    yield SE_ORBIT.format(semimajoraxis=semimajoraxis, eccentricity=eccentricity,
                          inclination=inclination + (180 if retrograde else 0), angle=angle)
